...
FROM dell_tests.countries_of_the_world t1 INNER JOIN dell_tests.covid_json_data t2
...
SELECT * FROM (
	SELECT DISTINCT
		t1.Country,
		SUM(CAST(t2.cumulative_count AS INTEGER)) as number_cases,
...
SELECT
	t1.region,
	SUM(CAST(t2.cumulative_count AS INTEGER)) AS number_of_cases,
